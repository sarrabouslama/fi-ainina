...
@pytest.mark.asyncio
async def test_incoming_ws_event_accepts_event_type_payload(db_sessionmaker, monkeypatch):
    mock_broadcast = AsyncMock()
    monkeypatch.setattr('app.microservices.alerts_ws.manager.broadcast', mock_broadcast)
    ...
    async with db_sessionmaker() as db:
        saved = (await db.execute(select(Alert).where(Alert.user_id == 'u-456'))).scalars().first()
        assert saved is not None
        assert saved.alert_type == 'fall_detected'
